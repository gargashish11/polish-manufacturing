...
#--------------------------------------------------------------------------
# yr1 <- read.arff('./1year.arff')
# yr2 <- read.arff('./2year.arff')
# yr3 <- read.arff('./3year.arff')
# yr4 <- read.arff('./4year.arff')
# yr5 <- read.arff('./5year.arff')
...
if (!file.exists('./rds/yr_pmm_OS_rf_cv.rds'))
{
  ...
  yr_pmm_OS.rf.cv <- train(class ~ ., data = yr_pmm_OS.train,
                           ...
                           tuneGrid = tune.grid)
  stopCluster(cl)
  saveRDS(yr_pmm_OS.rf.cv,'./rds/yr_pmm_OS_rf_cv.rds')
}

yr_pmm_OS.rf.cv <- readRDS('./rds/yr_pmm_OS_rf_cv.rds')
